...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusshunshazhaoCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusshunshazhaoCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusshunshazhao');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusshunshazhao');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusshunshazhao', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusshunshazhao', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusshunshazhao/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusshunshazhao', this.detail);
				this.$utils.msg('审核成功');
