...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusshunqingcehuaCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusshunqingcehuaCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusshunqingcehua');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusshunqingcehua');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusshunqingcehua', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusshunqingcehua', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusshunqingcehua/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusshunqingcehua', this.detail);
				this.$utils.msg('审核成功');
