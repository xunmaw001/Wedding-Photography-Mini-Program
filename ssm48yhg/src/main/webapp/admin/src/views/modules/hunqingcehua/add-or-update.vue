    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="流程简介" prop="liuchengjianjie">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="流程简介"
                  v-model="ruleForm.liuchengjianjie" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.liuchengjianjie" label="流程简介" prop="liuchengjianjie">
                    <span>{{ruleForm.liuchengjianjie}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="婚庆详情" prop="hunqingxiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.hunqingxiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.hunqingxiangqing" label="婚庆详情" prop="hunqingxiangqing">
                    <span v-html="ruleForm.hunqingxiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	clicknum : false,
      ...
      rules: {
          hunqingbiaoti: [
                { required: true, message: '婚庆标题不能为空', trigger: 'blur' },
          ],
          hunqingfengmian: [
          ],
          leixing: [
  ...
  methods: {
	...
	this.ruleForm.hunqingxiangqing = this.ruleForm.hunqingxiangqing.replace(reg,'../../../ssm48yhg/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `hunqingcehua/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.hunqingcehuaCrossAddOrUpdateFlag = false;
						 this.parent.search();
