...
export default {
  data() {
	...
	xiangpian : false,
      ...
      rules: {
          cehuazhanghao: [
                { required: true, message: '策划账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          cehuaxingming: [
                { required: true, message: '策划姓名不能为空', trigger: 'blur' },
          ],
          xingbie: [
          ],
          youxiang: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `cehuashi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.cehuashiCrossAddOrUpdateFlag = false;
						 this.parent.search();
